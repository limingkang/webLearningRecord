## 对比
我们团队在选型微前端框架时，调研了市面上实现微前端的框架，可供选择的有iframe、sigle-spa、qiankun和microApp。single-spa太过于基础，对原有项
目的改造过多，成本太高； iframe在所有微前端方案中是最稳定的、上手难度最低的，但它有一些无法解决的问题，例如性能低、通信复杂、双滚动条、弹窗无法全局
覆盖，它的成长性不高，只适合简单的页面渲染。剩下的只有qiankun和microApp了,简单自己实现一个microapp
